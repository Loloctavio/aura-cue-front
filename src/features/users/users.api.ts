...
import { safeSpotifyAuthorizationUrl } from "../../lib/externalUrls";
...
export async function changePassword(payload: ChangePasswordPayload): Promise<TokenResponse> {
  const { data } = await api.put<TokenResponse>("/users/me/password", payload);
  return data;
}
...
export async function getSpotifyConnectUrl(redirectTo = "/dashboard"): Promise<string> {
  ...
  const url = safeSpotifyAuthorizationUrl(data.authorization_url);
  if (!url) throw new Error("Invalid Spotify authorization URL");
  return url;
}
